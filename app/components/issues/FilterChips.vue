...
const { exclusions, toggleStatus: toggleExclusionStatus, togglePriority: toggleExclusionPriority, toggleType: toggleExclusionType, toggleLabel: toggleExclusionLabel, toggleAssignee: toggleExclusionAssignee, hasActiveExclusions } = useExclusionFilters()
...
<template>
  <div v-if="hasFilters" class="flex flex-wrap items-center gap-1.5">
    ...
    <!-- Inclusion filters (normal opacity) -->
    <div
    ...
    <!-- Exclusion filters (with opacity) -->
    <template v-if="hasActiveExclusions">
      <span class="text-[10px] text-muted-foreground uppercase tracking-wide ml-2">Hidden:</span>
